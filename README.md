...
- 天気も同時取得: `data/weather/open-meteo/<UTC日付>.ndjson`(舞浜1点。`sabotage-backfill` が
  `weather` テーブルへ流し込む)。
...
- **天気(Open-Meteo・キー不要)**(`data/weather.py`): 舞浜の気温・降水・**降水確率(今以降2時間の
  最大)**をポーラー/毎時フライホイールが待ち時間と一緒に記録する(`weather` テーブル)。
  降水確率が上がる = 屋内退避で室内系がこれから混む**先行シグナル**(雨の再配分イベントの予告)。
  現況ボードに気温バッジと「☔️ まもなく雨 → 室内混雑注意」を表示。早く貯めるほど後で学習に効く。
- **蓄積履歴の起動時ブートストラップ**(`data/history.py`): Streamlit Cloud の DB は揮発
  (再デプロイで空)なので、そのままだと到着時予測が「履歴薄→flat(現在値)」に戻る。一方
  `data` ブランチには毎時フライホイールが約1ヶ月分を貯めている。起動時にその NDJSON を
  GitHub raw(public なので認証不要)から取り込み、既存 `backfill` で揮発 DB に流し込む。
  これで**初回描画から平常回帰予測が効く**(バックテストで到着時MAE 4.6分 < 素朴6.4分=約28%改善)。
  既に十分な観測があるDB(常時稼働ポーラー等)ではスキップ(冪等)。
- **食事どきナビ**(`analysis/dining.py`): 「乗った数」ではなく着席・飲食時間の最大化(目的関数)の
  実装。**人圧(待ち時間総和=実データ)+天気**から「今は食べ時(混雑ピークは並ぶのが最も損)/
  乗り時(空いてる)/☔️室内へ」を助言。店は**静的な参考データ**(名前・エリア・室内か・提供形態)を
  持ち、実シグナルに掛けて「いつ・どこで食べると得か」だけを出す。
  **正直な限界**: 非公式APIは東京のレストランを持たない(`/children` で確認・0件)ため、店の
  **待ち時間・空席・メニュー・営業時刻は非提供**(無いものは出さない)。ショーも同様に時刻データが
  空(`/live` が `liveData: []`)だったため未実装。
